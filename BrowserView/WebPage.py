...
from config import ISSUEINITIAL, COMMITADDRESS
import os
...
def TableForWeb(crg:CodeReviewGenerator):
        ...
        for commit, details in table.items():
            i=len(details)
            j = i//2
            tablehtml+=f'\n\t<tr>\n\t\t<td><a href="{COMMITADDRESS}{commit}" target="_blank">{commit}</a></td>\n\t\t<td>'
...
def __createhtmlfile__(crg: CodeReviewGenerator):
  ...
  <!DOCTYPE html>
    ...
    I have implemented the changes regarding <r>story/defect</r> <a href="{id}" target="_blank">{branch}</a></br>
    ...
    html=html.format(style=style,table=table,id=ISSUEINITIAL+crg.git.my_branch.split("_")[0],branch=crg.git.my_branch,repo=rep)
